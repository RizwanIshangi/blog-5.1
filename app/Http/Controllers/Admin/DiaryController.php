...
class DiaryController extends Controller
{
    ...
    public function getAll()
    {
        ...
        if(Auth::user()->role=='admin'){
            $diary=Diary::with(['category', 'tags'])->orderBy('created_at', 'desc')->paginate(5);
            $view='admin.diary.alldiary';
        }else{
            $diary=Diary::with(['category', 'tags'])->where('user_id', Auth::user()->id)->orderBy('created_at', 'desc')->paginate(5);
