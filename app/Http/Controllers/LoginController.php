...
class LoginController extends Controller
{
   ...
   protected function login($instance){
       if($instance['email']) {
           $user=User::where('email', $instance['email']);
       }else {
      ...
      if($user->exists()){
          $user=$user->first();
          $updateUser=User::where('id', $user->id)->update(['image'=>$instance['avatar']]);
          if(Auth::loginUsingId($user->id)){
            return true;
          }

      }else{
        $pwd=explode('@', $instance['email']);
        $pwd=Hash::make($pwd[0].uniqid());
        $newUser=new User;
        $newUser->name=$instance['name'];
        $newUser->email=$instance['email'];
        $newUser->image=$instance['avatar'];
   ...
   public function callbackGithub(Request $req){
    ...
    $user=array('email'=>'rizwan.ishangi@gmail.com','name'=>'rizvi','avatar'=>'assets/img/avatar.png');
     if($this->login($user)){
         if(session('_next')){
             $next=session('_next');
             $req->session()->forget('_next');
